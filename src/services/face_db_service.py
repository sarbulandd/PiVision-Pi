...
class FaceDatabaseService:
    ...
    def get_all_faces(self) -> list:
        return [{"id": f["id"], "name": f["name"]} for f in self._load()]

    def get_encodings(self) -> list:
        return [{"name": f["name"], "encoding": f["encoding"]} for f in self._load()]

    def add_face(self, name: str, image_path: Path) -> dict | None:
        image = face_recognition.load_image_file(str(image_path))
        encodings = face_recognition.face_encodings(image)
        if not encodings:
            return None
        faces = self._load()
        entry = {"id": str(uuid.uuid4()), "name": name, "encoding": encodings[0]}
        faces.append(entry)
        self._save(faces)
